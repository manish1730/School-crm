  ...
  return (
    <div className="rounded-2xl bg-white p-4 shadow-sm space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-5">
        <Select required label="Class" value={filters.className} options={classes} onChange={(value) => setFilters({ ...filters, className: value, sectionName: "" })} />
        <Select required label="Section" value={filters.sectionName} options={sections} onChange={(value) => setFilters({ ...filters, sectionName: value })} />
        <Select required label="Exam Type" value={filters.examType} options={examTypes.map((item) => item.examName)} onChange={(value) => setFilters({ ...filters, examType: value })} />
        <Select required label="Subject" value={filters.subject} options={subjects.map((item) => item.subjectName)} onChange={(value) => setFilters({ ...filters, subject: value })} />
        <Input required label="Maximum Marks" type="number" value={filters.maximumMarks} onChange={(value) => setFilters({ ...filters, maximumMarks: value })} />
      </div>
      <button onClick={fetchStudents} className="bg-blue-900 text-white px-6 py-2 rounded-lg cursor-pointer">Load Students</button>
      ...
      {students.length > 0 && <div className="flex justify-end"><button onClick={handleSaveMarks} className="bg-blue-900 text-white px-6 py-2 rounded-lg cursor-pointer">Save Marks</button></div>}
    </div>
  );
}
