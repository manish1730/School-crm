...
export default function LeaveManagement() {
  ...
  return (
    <div className="space-y-4">
      ...
      {showModal && <MasterModal title="Add Leave Request" footer={<div className="flex justify-end gap-3"><button onClick={() => setShowModal(false)} className="bg-gray-200 px-4 py-2 rounded-lg">Cancel</button><button onClick={handleAddLeave} className="bg-blue-900 text-white px-5 py-2 rounded-lg">Save</button></div>}><div className="space-y-4 mb-4"><Select label="Staff" value={formData.staffId} options={staff.map((item) => ({ label: item.fullName, value: item._id }))} onChange={(value) => setFormData({ ...formData, staffId: value })} /><Input label="Leave Type" value={formData.leaveType} onChange={(value) => setFormData({ ...formData, leaveType: value })} /><Input label="From Date" type="date" value={formData.fromDate} onChange={(value) => setFormData({ ...formData, fromDate: value })} /><Input label="To Date" type="date" value={formData.toDate} onChange={(value) => setFormData({ ...formData, toDate: value })} /><Input label="Reason" value={formData.reason} onChange={(value) => setFormData({ ...formData, reason: value })} /></div>{error && <div className="mb-4 p-3 rounded-lg bg-red-100 text-red-600">{error}</div>}</MasterModal>}
    </div>
  );
}
